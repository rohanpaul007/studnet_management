            ...
            case 'update_attendance':
                ...
                case 'get_attendance':
                    ...
                    echo json_encode([
                        ...
                        'date_queried' => $date // For debugging
                    ]);
                    exit;
...
<div class="sidebar_container" id="sidebar">
    <ul class="side_menu">
        ...
        <li><a href="student_managment.php"><i class="fa-solid fa-user-graduate"></i> <span>Manage Students</span></a></li>
        <li><a href="attendence.php" class="active"><i class="fa-solid fa-clipboard-check"></i> <span>Attendance</span></a></li>
        <li><a href="class_managment.php"><i class="fa-solid fa-chalkboard-teacher"></i> <span>Class Management</span></a></li>
        <li><a href="#"><i class="fa-solid fa-user-tie"></i> <span>Teachers</span></a></li>
    </ul>
</div>
                    ...
                    <div class="row g-4" id="attendanceCardsContainer">
                        ...
                        <div class="col-12 col-sm-6 col-md-4 col-lg-3" >
                            <div class="card h-100 shadow-sm student-card w-100 mb-3" data-student-id="<?= $data['id'] ?>" >
                                <div class="card-body text-center p-5 w-100" style="margin-bottom:25px;marin-right:350px;">
                                    <!-- Student Image -->
                                    <div class="mb-3">
    ...
    <div class="col-12">
        <div class="card shadow-sm">
            <div class="card-header bg-primary text-white">
                <h5 class="mb-0">Attendance Report for <?= date('d F Y', strtotime($attendance_date)) ?></h5>
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            <?php if (!empty($attendance_report)): ?>
                                ...
                                <tr>
                                    ...
                                    <td data-label="Name"><?= htmlspecialchars($data['student_name']) ?></td>
                                    <td data-label="Roll"><?= $data['student_roll'] ?></td>
                                    <td data-label="Class"><?= $data['student_grade'] ?>-<?= $data['student_section'] ?></td>
                                    <td data-label="Status">
                                        <span class="badge badge-<?= ($data['status'] ?? '') == 'Present' ? 'success' : 'danger' ?>">
                                            <?= $data['status'] ?? 'Not Marked' ?>
                                        </span>
                                    </td>
                                </tr>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="6" class="text-center">No attendance data found for this date</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
...
<div class="container mt-4">
            ...
            <div class="card shadow-sm">
                <div class="card-body">
                    ...
                    <select id="student_select" class="form-control">
                        ...
                        <?php foreach ($student->getAllStudents() as $studentData): ?>
                            <option value="<?= $studentData['id'] ?>">
    ...
    <div class="row mt-4">
        <div class="col-12">
            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h5 id="report_title" class="mb-0">Monthly Attendance Report</h5>
                </div>
                <div class="card-body">
                    <div id="monthly_report_container" class="table-responsive">
                        <!-- Dynamic content will be loaded here -->
                        <p class="text-center text-muted">Select a student to view their monthly report.</p>
                    </div>
                </div>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    loadAttendanceData($('#attendance_date').val());
    ...
    $(document).on('click', '.mark-present, .mark-absent', function() {
        ...
        // Then send to server
        updateAttendance(studentId, status, element);
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response && response.success) {
                    ...
                    renderAttendanceTable(response.data, date);
                } else {
                    showEmptyState(date, response?.error);
                }
            },
            error: function(xhr) {
    ...
    function renderAttendanceTable(data, date) {
        ...
        $('.card-header h5').text(`Attendance Report - ${displayDate}`);
    }
    ...
    function showEmptyState(date, error) {
        const displayDate = formatDisplayDate(date);
        $('#attendanceCardsContainer').html(`
            <div class="col-12">
                <div class="alert alert-warning text-center">
                    ${error || 'No attendance data available for ' + displayDate}
                </div>
            </div>
        `);
        $('#attendanceTable tbody').html(`
            <tr>
                <td colspan="6" class="text-center py-4 text-muted">
                    ${error || 'No records found for ' + displayDate}
                </td>
            </tr>
        `);
    }
    ...
    function updateAttendance(studentId, status, element) {
